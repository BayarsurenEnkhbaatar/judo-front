    ...
    <Link to={`/comptation-right/${data.id}`}>
       <div className='shadow-lg font-Roboto rounded-md hover:shadow-xl hover:bg-gray-100'>
            <div className='rounded-t-md py-2 bg-blue-600'>
                <h1 className='text-center text-white uppercase text-lg '>{data.name}</h1>
            </div>
            <div className='p-4'>
                ...
                <p className='text-xs mt-4 text-center md:text-xs font-bold'>{new Date(data.start_date).getFullYear()} оны {new Date(data.start_date).getMonth() +1} сарын {new Date(data.start_date).getDate()}-{new Date(data.end_date).getDate()} ны хооронд</p>
